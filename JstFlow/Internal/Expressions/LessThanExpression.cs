...
namespace JstFlow.Internal.Expressions
{
    public class LessThanExpression : IExpression
    {
        ...
        public string ExpressionName => "小于比较";
        ...
        public async Task<Res<Label<IValue>>> Evaluate(IDictionary<Label, IValue> inputs)
        {
            try
            {
                ...
                {
                    return Res<Label<IValue>>.Fail("小于比较只支持数字类型");
                }

                // 执行小于比较
                var leftNumber = leftValue.GetValue<decimal>();
                var rightNumber = rightValue.GetValue<decimal>();
                bool isLess = leftNumber < rightNumber;

                // 创建结果
                var resultValue = new BoolValue(isLess);
                var result = new Label<IValue>("result", "比较结果", resultValue);

                return Res<Label<IValue>>.Ok(result, "小于比较执行成功");
            }
            catch (Exception ex)
            {
                return Res<Label<IValue>>.Fail($"小于比较执行失败: {ex.Message}");
            }
        }
    }
} 