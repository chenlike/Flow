...
namespace JstFlow.Internal.Expressions
{
    public class GreaterThanExpression : IExpression
    {
        ...
        public string ExpressionName => "大于比较";
        ...
        public async Task<Res<Label<IValue>>> Evaluate(IDictionary<Label, IValue> inputs)
        {
            try
            {
                ...
                {
                    return Res<Label<IValue>>.Fail("大于比较只支持数字类型");
                }

                // 执行大于比较
                var leftNumber = leftValue.GetValue<decimal>();
                var rightNumber = rightValue.GetValue<decimal>();
                bool isGreater = leftNumber > rightNumber;

                // 创建结果
                var resultValue = new BoolValue(isGreater);
                var result = new Label<IValue>("result", "比较结果", resultValue);

                return Res<Label<IValue>>.Ok(result, "大于比较执行成功");
            }
            catch (Exception ex)
            {
                return Res<Label<IValue>>.Fail($"大于比较执行失败: {ex.Message}");
            }
        }
    }
} 